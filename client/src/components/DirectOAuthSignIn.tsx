...
export const DirectOAuthSignIn: React.FC = () => {
  const handleGoogleSignIn = () => {
    // Direct Google OAuth without Stack Auth components
    // Using Stack Auth's OAuth endpoint but bypassing their UI components
    const googleOAuthUrl = `https://accounts.google.com/oauth2/auth?` +
      `client_id=88513373775-mu3geg0faa34udhpp3dd4uh969d7g307.apps.googleusercontent.com&` +
      `redirect_uri=${encodeURIComponent('https://api.stack-auth.com/api/v1/auth/oauth/callback/google')}&` +
      `response_type=code&` +
