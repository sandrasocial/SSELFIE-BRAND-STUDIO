...
function SimpleTraining() {
  ...
  // ✅ GENDER SELECTION STATE - Required before training starts
  ...
  // ✅ CHECK USER'S EXISTING GENDER - Load from user profile
  const { data: userProfile } = useQuery<{ gender?: string }>({
  ...
  // Initialize gender state from user profile
  useEffect(() => {
    if (userProfile?.gender) {
      console.log('✅ GENDER: Found existing gender in profile:', userProfile.gender);
  ...
  const saveGenderMutation = useMutation({
    mutationFn: async (gender: string) => {
      const response = await apiRequest('/api/update-user', 'POST', {
        gender: gender
      });
      return response;
    },
    onSuccess: (data: any) => {
      console.log('✅ GENDER: Saved successfully to database');
      setGenderCaptured(true);
      toast({
        title: "Perfect!",
        description: `Got it - you're ${userGender === 'woman' ? 'a woman' : userGender === 'man' ? 'a man' : 'set'}. Now let's train your AI with your photos.`,
      });
    },
    onError: (error: any) => {
      console.error('❌ GENDER: Failed to save:', error);
  ...
  const handleGenderSelection = async (gender: string) => {
    console.log('🎯 GENDER: User selected:', gender);
    setUserGender(gender);
    setIsSavingGender(true);
    
    // Save to database immediately
    saveGenderMutation.mutate(gender);
  };
  ...
  // ✅ STEP 1: GENDER SELECTION - Required before training starts
  if (!genderCaptured && isAuthenticated) {
    return (
      <div style={{ 
        minHeight: '100vh', 
        background: '#ffffff',
        ...
        <section style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              Step 1 of 2: Model Configuration
            </div>
            ...
            <p style={{
              ...
              Are you a man or woman? This ensures we generate photos that look like you with the right styling and technical settings.
            </p>
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  padding: '24px 32px',
                  background: isSavingGender ? '#f5f5f5' : '#ffffff',
                  ...
                  textTransform: 'uppercase',
                  cursor: isSavingGender ? 'not-allowed' : 'pointer',
                  transition: 'all 0.3s ease',
                  fontFamily: 'inherit',
                  opacity: isSavingGender ? 0.5 : 1
                }}
                onMouseEnter={(e) => {
                  if (!isSavingGender) {
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
                  <span>Woman</span>
                  <small style={{ opacity: 0.7, fontSize: '12px', textTransform: 'none' }}>Train AI for female photos</small>
                </div>
              </button>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
                  <span>Man</span>
                  <small style={{ opacity: 0.7, fontSize: '12px', textTransform: 'none' }}>Train AI for male photos</small>
                </div>
              </button>
            </div>
            ...
            <div style={{
              ...
              This information helps us train your AI model with the right styling approach and technical parameters for professional results.
            </div>
          </div>
        </section>
