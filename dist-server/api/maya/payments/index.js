...
import { mayaPayments } from '../../../shared/schema-maya.js';
...
const PLAN_PRICES = {
    basic: { monthly: 2900, yearly: 31900 },
    pro: { monthly: 4900, yearly: 53900 },
    enterprise: { monthly: 9900, yearly: 108900 },
};
...
async function handleGetPayments(req, res, userId) {
    try {
...
async function handleCreatePaymentSession(req, res, userId) {
    try {
...
async function handleUpdatePayment(req, res, userId) {
    try {
        ...
        const [updatedPayment] = await db
            ...
            .where(and(eq(mayaPayments.id, parseInt(paymentId)), eq(mayaPayments.userId, userId)))
            .returning();
        if (!updatedPayment) {
...
async function handleCancelSubscription(req, res, userId) {
    try {
        const activePayment = await db.select()
            .from(mayaPayments)
            .where(and(eq(mayaPayments.userId, userId), eq(mayaPayments.subscriptionStatus, 'active')))
            .limit(1);
