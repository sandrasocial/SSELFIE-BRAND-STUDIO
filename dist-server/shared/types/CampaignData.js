export class CampaignManager {
    ...
    getPerformanceMetrics(campaignId) {
        ...
        const recommendations = [];
        if (kpis.roas < 5.0)
            recommendations.push("Increase ROAS with better audience targeting");
        if (kpis.ctr < 0.015)
            recommendations.push("Improve ad creative with Sandra's authentic voice");
        if (kpis.conversion_rate < 0.02)
            recommendations.push("Optimize landing page user experience");
        if (kpis.profit_margin < 80)
            recommendations.push("Reduce acquisition costs while maintaining quality");
