...
export const userPersonalBrand = pgTable('user_personal_brand', {
    ...
    personalityTraits: jsonb('personality_traits').$type().default([]), // Core personality characteristics
    ...
    photoUsageGoals: jsonb('photo_usage_goals').$type().default([]), // Social media, website, marketing, etc.
...
export const userStyleProfile = pgTable('user_style_profile', {
    ...
    styleCategories: jsonb('style_categories').$type().default([]), // Editorial, Minimalist, Boho, Corporate, etc.
    colorPreferences: jsonb('color_preferences').$type().default({ primaryColors: [], accentColors: [], avoidColors: [] }),
    // Settings & Environments
    settingsPreferences: jsonb('settings_preferences').$type().default([]), // Office, home, outdoor, studio, etc.
    locationVibes: jsonb('location_vibes').$type().default([]), // Luxury, natural, urban, cozy, etc.
    ...
    styleAvoidances: jsonb('style_avoidances').$type().default([]), // What they absolutely don't want
    boundariesAndLimits: text('boundaries_and_limits'), // Personal or cultural considerations
    // Inspiration & References
    inspirationImages: jsonb('inspiration_images').$type().default([]), // URLs or descriptions of inspiring looks
    styleIcons: jsonb('style_icons').$type().default([]), // People whose style they admire
    brandReferences: jsonb('brand_references').$type().default([]), // Brands they love or aspire to
    // Timestamps
    createdAt: timestamp('created_at').defaultNow(),
    updatedAt: timestamp('updated_at').defaultNow(),
});
...
export const mayaPersonalMemory = pgTable('maya_personal_memory', {
    ...
    preferredTopics: jsonb('preferred_topics').$type().default([]), // Business, style, personal growth, etc.
    ...
    successfulPromptPatterns: jsonb('successful_prompt_patterns').$type().default([]), // Prompts that generated great results
