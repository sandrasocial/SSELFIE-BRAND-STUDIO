export class EmailInterventionCampaign {
    ...
    static getPlanDisplayName(plan) {
        const planMap = {
    ...
    static getDaysSinceSignup(createdAt) {
        const signupDate = new Date(createdAt);
        const now = new Date();
        return Math.floor((now.getTime() - signupDate.getTime()) / (1000 * 60 * 60 * 24));
    }
    static getEmailTemplate(user, name, planName, daysSinceSignup) {
