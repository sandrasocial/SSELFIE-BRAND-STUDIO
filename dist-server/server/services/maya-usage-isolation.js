    ...
    async getPopularMemberCategories() {
        try {
            ...
            const categories = memberChats.map(chat => {
                if (chat.chatTitle?.includes('Headshot'))
                    return 'Professional Headshots';
                if (chat.chatTitle?.includes('Social'))
                    return 'Social Media Photos';
                if (chat.chatTitle?.includes('Website'))
                    return 'Website Photos';
                if (chat.chatTitle?.includes('Email'))
                    return 'Email & Marketing Photos';
                if (chat.chatTitle?.includes('Premium'))
