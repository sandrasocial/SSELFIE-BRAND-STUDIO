...
router.post('/monitor/start', requireStackAuth, async (req, res) => {
    try {
        ...
        await SlackNotificationService.sendAgentInsight('ava', 'operational', 'Email Monitoring Started', `Automated email monitoring activated for user ${userId}. Will check for new emails every ${intervalMinutes} minutes.`, 'medium');
        res.json({
...
router.post('/test-processing', requireStackAuth, async (req, res) => {
    try {
        ...
        await SlackNotificationService.sendAgentInsight('ava', 'strategic', 'Email Processing Test Complete', `🧪 **Test Results:**\n\n` +
            ...
            `• Use AI-generated response drafts`, 'high');
        res.json({
