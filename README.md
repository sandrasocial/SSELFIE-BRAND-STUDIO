...
The User Journey
Discover (The Conversation): Natural conversation with Maya to define brand strategy
Train (One-Time Setup): Upload selfies to create personalized LoRA model
Create (The Studio): Generate unlimited photos and videos based on strategic concepts
Collect (The Gallery): Build valuable library of on-brand content over time
Quick Start
Setup: npm install && npm run dev
Database: Configured with NeonDB
Environment: Copy .env.example to .env and configure API keys
Development: Visit http://localhost:5000
