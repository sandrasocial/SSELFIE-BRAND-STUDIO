...
## Recent Changes
- **Maya Emoji Styling System RESTORED** (Aug 30, 2025): Fixed critical frontend bug where Maya's emoji-coded styling system was being stripped before reaching the backend. Maya uses specific emojis (✨💫🔥🌟💎🌅🏢💼🌊👑💃📸🎬) in concept titles to communicate styling approaches to the backend for intelligent prompt generation. The frontend was incorrectly removing these emojis with `conceptName.replace(/[✨💫💗🔥🌟💎🌅🏢💼🌊👑💃📸🎬]/g, '')`, breaking Maya's ability to identify which styling approach to use. Now preserves the complete concept name with emojis, allowing Maya's styling intelligence to flow properly from concept creation to image generation. This fixes the disconnect between Maya's emoji-coded styling system and actual generation parameters.
