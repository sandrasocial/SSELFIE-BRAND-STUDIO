...
- **Maya Emoji Usage Enforcement STRENGTHENED** (Aug 30, 2025): Maya was not following emoji styling instructions in concept titles despite having detailed emoji system. Strengthened prompt with "MANDATORY: NEVER CREATE CONCEPTS WITHOUT EMOJIS!" and "YOU MUST ALWAYS include styling emojis at the start of EVERY concept title". Updated instructions to require emoji format for all concept titles to ensure proper styling communication throughout the generation system. System creates concepts with embedded FLUX prompts, stores them in database with fullPrompt field preserved, and generation route now correctly uses embedded prompts for perfect consistency between concept descriptions and generated images.
