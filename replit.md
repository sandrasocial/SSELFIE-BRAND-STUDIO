...
- **Maya Single API Call Consistency System CRITICAL BUG FIX** (Aug 30, 2025): Identified and fixed critical bug causing Maya's consistency issues. Found duplicate embedded prompt checks where the second check was examining wrong variable (userConcept.fullPrompt instead of conceptCard.fullPrompt). This caused ALL generations to fall back to dual API calls instead of using embedded prompts. Fixed by removing incorrect duplicate check at line 668, ensuring only the proper database-retrieved conceptCard.fullPrompt is used. Single API call system now properly detects embedded prompts from concept cards and uses them directly for generation, eliminating the dual API inconsistency completely. System creates concepts with embedded FLUX prompts, stores them in database with fullPrompt field preserved, and generation route now correctly uses embedded prompts for perfect consistency between concept descriptions and generated images.
