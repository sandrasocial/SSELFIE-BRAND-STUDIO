...
## Recent Changes
- **Maya Styling Inconsistency CRITICAL FIX COMPLETED** (Aug 30, 2025): Identified and resolved the core issue causing disconnect between Maya's frontend concept descriptions and actual FLUX prompts sent to Replicate. The problem was that Maya wasn't generating embedded `FLUX_PROMPT:` format in single API call responses, causing system to fall back to dual API call system which generated different styling. **SOLUTION IMPLEMENTED**: Updated Maya's system prompt with explicit FLUX_PROMPT format requirements and concrete examples, enhanced pattern matching regex for better FLUX_PROMPT detection, added intelligent fallback system that creates FLUX prompts from concept content when format not followed, and comprehensive debugging to track FLUX_PROMPT generation. **VALIDATION**: Testing confirms Maya now generates concepts with proper embedded FLUX prompts (e.g., "The Power Coffee Boss" with matching FLUX_PROMPT), ensuring perfect consistency between concept descriptions and generated images.
