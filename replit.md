...
- **Maya Historical Concept Cards Fix (August 29, 2025)**: Fixed critical issue where previous Maya chat sessions displayed raw JSON instead of rendered concept cards. Enhanced Maya chat messages database schema with dedicated conceptCards, quickButtons, and canGenerate fields. Updated storage logic to save concept cards separately from content and enhanced retrieval to properly parse JSON fields for frontend compatibility. Historical conversations now correctly display interactive concept cards instead of code.
- **Maya Editorial Style Audit & Upgrades (August 29, 2025)**: Conducted comprehensive audit of Maya page against editorial style guide and implemented sophisticated design upgrades. Enhanced chat header with editorial eyebrow text and Times New Roman typography. Upgraded welcome state with larger Maya avatar, hover effects, and improved spacing. Enhanced input area with better focus states, typography, and editorial button design. Added interactive progressive session loading to replace static "more sessions" text. Fixed Maya Claude API connection by updating incorrect model name from 'claude-sonnet-4-20250514' to 'claude-3-5-sonnet-20241022'. Maya interface now fully matches editorial style guide with elegant hover animations and professional typography.
- **Maya Pipeline Optimization Complete (August 30, 2025)**: Successfully completed Phase 2 pipeline optimization connecting Maya's styling intelligence directly to image generation. Enhanced createDetailedPromptFromConcept function with category-aware styling that uses Maya's 19 categories and 5 styling approaches each. Added intelligent category detection from user context (Business, Lifestyle, Casual, Travel, Instagram). Cleaned up redundant code and fixed LSP errors. Comprehensive testing confirms Maya's specific styling descriptions now flow from personality file through to FLUX prompts, eliminating generic fallbacks. All 19 categories generate unique, professional styling approaches. End-to-end pipeline working with successful prediction generation for all test categories.
- **Maya Prompt Flow Cleanup Complete (August 30, 2025)**: Completed comprehensive 5-task prompt flow cleanup implementation to eliminate formatting interruptions and create natural, flowing FLUX prompts. Updated Claude API instructions in createDetailedPromptFromConcept to prevent formatting elements like "FLUX 1.1 PRO STYLING PROMPT:" and bracketed annotations. Enhanced cleanMayaPrompt function with regex patterns to remove [Shot type:], [Environment:], [Lighting:], and other formatting artifacts. Added removal of conversational elements like "I can help you" and "Here's". Verified Maya's personality file and PersonalityManager buildNaturalPrompt function contain no harmful formatting instructions. Multi-layered protection approach: source prevention (Claude API) + enhanced cleaning (cleanMayaPrompt) + system verification (personality files). All components validated and production-ready. Fixed 401 authentication polling errors with graceful error handling that stops polling silently on session timeout. Complete implementation: Task 1 (source instructions), Task 2 (enhanced cleaning), Task 3 (personality verification), Task 4 (PersonalityManager verification), Task 5 (validation testing).
