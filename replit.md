...
- **Maya Prompt Flow Cleanup Complete (August 30, 2025)**: Implemented comprehensive prompt flow cleanup to eliminate formatting interruptions and create natural, flowing FLUX prompts. Updated Claude API instructions in createDetailedPromptFromConcept to prevent formatting elements like "FLUX 1.1 PRO STYLING PROMPT:" and bracketed annotations. Enhanced cleanMayaPrompt function with regex patterns to remove [Shot type:], [Environment:], [Lighting:], and other formatting artifacts. Added removal of conversational elements like "I can help you" and "Here's". Multi-layered approach with source prevention and enhanced cleaning ensures clean, continuous styling descriptions that flow naturally from technical prefix without template formatting. Verified Maya's personality file contains no harmful formatting instructions. Fixed 401 authentication polling errors with graceful error handling that stops polling silently on session timeout. All three tasks completed: source instruction updates, enhanced cleaning function, and personality file verification.
