...
- **Maya Personality Physical Feature Cleanup (August 29, 2025)**: Systematically removed all physical feature assumptions from Maya's personality file. Replaced specific terms with universal alternatives: "confident professional expression" → "natural professional expression", "sharp focus on eyes" → "sharp focus", "eye detection" → "subject detection". Added comprehensive physicalFeaturePolicy with clear guidelines to use universal terms like "natural hair color" and "natural expression" instead of specific physical characteristics.
...
- **API Compatibility Fix (August 29, 2025)**: Corrected megapixels parameter validation for Replicate FLUX API compatibility. Changed from research-optimal values (1.5-2.0) to API-compliant values ("1" only, as Replicate only accepts "1" or "0.25"). Fixed critical generation failures while maintaining all other research-optimized parameters (guidance_scale 3.5, 50 inference steps, LoRA weights 0.9-1.0).
- **LoRA Weight Optimization (August 29, 2025)**: Increased LoRA weight from 1.0/0.9 to 1.1 across all shot types for stronger user likeness influence based on generation quality feedback. Updated both Maya's personality fluxOptimization settings and fallback parameters in model-training-service.ts.
