...
- **Maya Single API Call Consistency System COMPLETED** (Aug 30, 2025): Completed comprehensive investigation and full implementation of Maya's dual API call inconsistency solution. System now creates concept cards with embedded FLUX prompts in one call, eliminating inconsistencies between concept descriptions and generated prompts. Successfully tested across Business, Instagram, Travel, and GRWM categories. Maya creates concepts with format: user description + "FLUX_PROMPT: [technical prompt]" ensuring perfect consistency. Architecture complete with parsing logic, database persistence fix, and fallback system. Database now properly stores and retrieves fullPrompt field in concept cards JSONB. Generation route prioritizes embedded prompts from single API calls, with dual API backup. Single API call consistency solution fully operational and eliminates Maya's styling inconsistencies completely.
