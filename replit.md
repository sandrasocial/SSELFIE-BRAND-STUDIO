...
- **Maya Prompt Flow Cleanup Complete (August 30, 2025)**: Implemented comprehensive prompt flow cleanup to eliminate formatting interruptions and create natural, flowing FLUX prompts. Updated Claude API instructions in createDetailedPromptFromConcept to prevent formatting elements like "FLUX 1.1 PRO STYLING PROMPT:" and bracketed annotations. Enhanced cleanMayaPrompt function with regex patterns to remove [Shot type:], [Environment:], [Lighting:], and other formatting artifacts. Multi-layered approach ensures clean, continuous styling descriptions that flow naturally from technical prefix without template formatting. Fixed 401 authentication polling errors with graceful error handling that stops polling silently on session timeout.
