...
const metrics = {
  httpRequestDurationMicroseconds: new PrometheusClient.Histogram({
    name: http_request_duration_seconds,';    help: Duration of HTTP requests in seconds,';    labelNames: ['method', 'route', 'code']';  }),'
  
  aiAgentExecutionTime: new PrometheusClient.Histogram({
    name: ai_agent_execution_seconds,';    help: AI Agent execution time in seconds,';    labelNames: ['agent_name', 'task_type']';  }),'

  databaseQueryDuration: new PrometheusClient.Histogram({
    name: database_query_duration_seconds,';    help: Database query duration in seconds,';    labelNames: [query_type]';  }),'

  memoryUsage: new PrometheusClient.Gauge({
    name: node_memory_usage_bytes,';    help: Node.js memory usage in bytes,';  }),'

  activeUsers: new PrometheusClient.Gauge({
    name: active_users_total,';    help: Total number of active users,`;  })'
};
