...
router.get('/generated-images', isAuthenticated, async (req, res) => {
  try {
    ...
    // Fetch all AI images generated by Maya for this user
    const images = await storage.getAIImages(userId);
    
    // Filter for Maya-generated images and format for gallery
    const mayaImages = images
      .filter(img => img.source === 'maya-chat' || img.generatedPrompt)
      .map(img => ({
        id: img.id,
    ...
    console.error('Maya generated images fetch error:', error);
    res.status(500).json({ 
