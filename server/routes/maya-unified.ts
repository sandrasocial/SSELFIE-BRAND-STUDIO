...
✨ MAYA'S NATURAL EMOJI INTELLIGENCE:
...
// 🎯 NEW: INTENT-DRIVEN CHAT using Maya State Machine
router.post('/member/chat-state-machine', requireStackAuth, async (req: AdminContextRequest, res) => {
  const startTime = Date.now();
  
  try {
    const userId = req.userId;
    if (!userId) {
      logMayaAPI('/chat-state-machine', startTime, false);
      return res.status(401).json({ error: 'Authentication required' });
    }
    ...
    console.log('🧠 Maya State Machine: Processing message from user', userId);
    
    // Process message through Maya's intent-driven state machine
    const mayaResponse = await mayaStateMachine.processUserMessage(
      userId, 
      message.trim(), 
      conversationHistory
    );
    
    logMayaAPI('/chat-state-machine', startTime, true);
    
    res.json({
      success: true,
      message: mayaResponse.message,
      mode: mayaResponse.mode,
      conceptCards: mayaResponse.conceptCards || [],
      quickActions: mayaResponse.quickActions || [],
      onboardingUI: mayaResponse.onboardingUI,
      requiresUserInput: mayaResponse.requiresUserInput
    });
    ...
    res.status(500).json({
      success: false,
      message: 'I\'m having a small technical moment! Let me help you with that. What specifically are you looking for with your photos?',
      mode: 'conversation',
      conceptCards: [],
      quickActions: []
    });
