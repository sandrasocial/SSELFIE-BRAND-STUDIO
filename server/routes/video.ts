import { Router } from "express";import { requireStackAuth } from "..stack-auth.js";import { generateVeo3Video, getVeo3Status, getQualityPreset } from "..services/video/veo3";import { storage } from "..storage.js";import { generatedImages, aiImages } from "..../shared/schema";import { eq, and } from "drizzle-orm";import { db } from "..drizzle.js";"
const router = Router();
 ...
 */
router.post('/generate', requireStackAuth, async (req, res) => {';  try {'
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: User not authenticated })';    }'

    // Check if VEO3 is enabled
    if (!process.env.VEO3_ENABLED || process.env.VEO3_ENABLED !== '1') {';      return res.status(503).json({ '
        error: VEO 3 video generation is not enabled,';        details: Contact support for access to video generation features;      })';'
    }

    if (!process.env.GOOGLE_API_KEY) {
      return res.status(503).json({
        error: Video generation service not configured,';        details: Google API key not available;      })';'
    }

    const { imageId, motionPrompt, mode = 'preview', audioScript, aspectRatio = '9:16' } = req.body';'
    // Validate required parameters
    if (!motionPrompt || typeof motionPrompt !== 'string') {';      return res.status(400).json({ error: motionPrompt is required and must be a string })';    }'

    if (motionPrompt.trim().length < 8) {
      return res.status(400).json({ error: motionPrompt must be at least 8 characters long })';    }'

    if (!['preview', 'production'].includes(mode)) {";      return res.status(400).json({ error: mode must be either preview" or "production" })';    }"";'
    if (!['16:9', '9:16', '1:1'].includes(aspectRatio)) {";      return res.status(400).json({ error: aspectRatio must be 16:9", "9:16", or "1:1" })';    }"";'
    ...
    console.log('🎬 VEO 3: Generate request', {';      userId,'
      imageId,
    ...
    const videoRecord = await storage.saveGeneratedVideo({
      ...
      imageSource: imageId ? 'generated' : null,';      motionPrompt,'
      jobId: result.jobId,
      status: pending,';      progress: 0,'
      estimatedTime: result.estimatedTime
    });

    console.log('✅ VEO 3: Job started and saved', { ';      jobId: result.jobId, '
    ...
    const errorMessage = error instanceof Error ? error.message: Unknown error;    console.error('❌ VEO 3: Generate request failed', { ';      error: errorMessage,'
      userId: req.user?.id,
 ...
 */
router.get('/status/:jobId', requireStackAuth, async (req, res) => {';  try {'
    ...
    console.log('🔍 VEO 3: Status check', {';      jobId: jobId.slice(-20),'
      status: status.status,
    ...
    const errorMessage = error instanceof Error ? error.message: Unknown error;    console.error('❌ VEO 3: Status check failed', {';      jobId: req.params.jobId,'
      error: errorMessage,
      userId: req.user?.id
    });
...
router.get('/history', requireStackAuth, async (req, res) => {';  try {'
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: User not authenticated })';    }'
    ...
    console.log('📚 VEO 3: History request', { ';      userId, '
      count: videos.length,
      limit,
      offset
    });
    ...
    const errorMessage = error instanceof Error ? error.message: Unknown error;    console.error('❌ VEO 3: History request failed', {';      error: errorMessage,'
      userId: req.user?.id
    });

    res.status(500).json({
      error: Failed to fetch video history,';      details: errorMessage'
    });
  }
});
...
router.post('/save', requireStackAuth, async (req, res) => {';  try {'
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: User not authenticated })';    }'
    ...
    if (video.status !== 'completed' || !video.videoUrl) {';      return res.status(400).json({ error: Video is not ready to be saved })';    }'
    ...
    const errorMessage = error instanceof Error ? error.message: Unknown error;    console.error('❌ VEO 3: Save video failed', {';      error: errorMessage,'
      userId: req.user?.id,
      videoId: req.body.videoId
    });

    res.status(500).json({
      error: Failed to save video,';      details: errorMessage'
    });
  }
});
 ...
 */
router.get('/presets', requireStackAuth, async (req, res) => {';  try {'
    const presets = {
      preview: getQualityPreset('preview'),';      production: getQualityPreset('production')';    }';'

    res.json({ presets });
  } catch (error) {
    console.error('❌ VEO 3: Presets request failed', error)';    res.status(500).json({ error: Failed to fetch presets })';  }'
});
