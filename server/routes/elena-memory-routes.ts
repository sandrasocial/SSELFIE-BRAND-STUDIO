...
import { ElenaConversationDetectionService } from '../services/elena-conversation-detection';
...
router.post('/create-workflow', isAdminOrToken, async (req, res) => {
  try {
    ...
    // RESTORED: Use Elena conversation detection for memory-aware workflows
    const elenaMemory = ElenaConversationDetectionService.getInstance();
    const workflow = elenaMemory.detectWorkflowFromConversation(request, `memory-${Date.now()}`);
...
router.post('/execute-workflow', isAdminOrToken, async (req, res) => {
  try {
    ...
    // Execute workflow
    // RESTORED: Use Elena memory system for execution
    const elenaMemory = ElenaConversationDetectionService.getInstance();
    const workflow = elenaMemory.stagedWorkflows.get(workflowId);
    const result = { executionId: workflowId, status: 'executing' };
...
router.post('/workflow-progress/:workflowId', isAdminOrToken, async (req, res) => {
  try {
    ...
    // RESTORED: Use Elena memory system for progress tracking
    const elenaMemory = ElenaConversationDetectionService.getInstance();
    const workflow = elenaMemory.stagedWorkflows.get(workflowId);
    const progress = { status: workflow?.status || 'not_found', workflow };
...
export async function registerElenaMemoryRoutes(app: any) {
  app.use('/api/elena/memory', router);
  console.log('✅ Elena Memory Routes registered');
}
