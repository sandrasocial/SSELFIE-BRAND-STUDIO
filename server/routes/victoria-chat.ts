...
router.post('/api/victoria-website-chat', requireStackAuth, async (req: any, res) => {';  try {'
...
- Include "GENERATE_WEBSITE:  followed by JSON with:;  {"
    "businessName": "...","businessType": "...","businessDescription": "...","targetAudience": "...","keyFeatures": ["...", "..."],"brandPersonality": "...","contentStrategy": "...","selectedImages": ${JSON.stringify(selectedImages)},"selectedFlatlays": ${JSON.stringify(selectedFlatlays)},"editorialComponents": ["HeroFullBleed", "EditorialImageBreak", "MoodboardGallery"]`  }"
...
Maintain Victoria"s personality: professional, warm, design-focused, and luxury-minded. Use Times New Roman aesthetic references and editorial design language.``;;"
    ...
    const response = await fetch('http://localhost:5000/api/victoria-website-chat-internal', {';      method: POST,';      headers: {'
        'Content-Type': 'application/json','Authorization': req.headers.authorization';      },'
      body: JSON.stringify({
    ...
    if (victoriaResponse.response.includes('GENERATE_WEBSITE: )) {;      const jsonMatch = victoriaResponse.response.match(/GENERATE_WEBSITE:\s*({.*})/s)';'
      if (jsonMatch) {
        try {
          const websiteData = JSON.parse(jsonMatch[1]);
          
          // Generate website using Victoria's website generation API';          const websiteResponse = await fetch('http://localhost:5000/api/victoria/generate', {';            method: POST,';            headers: {'
              'Content-Type': 'application/json','Authorization': req.headers.authorization';            },'
    ...
    res.json({
      success: true,
      response: cleanResponse || 'I'd love to help you create your website! Tell me about your business and what you do.',`      website: generatedWebsite`    })`;'

  } catch (error) {
    console.error('Victoria chat error: , error);    res.status(500).json({'
      success: false,
