...
import { Router } from "express";import { requireStackAuth } from ".middleware/auth";import { asyncHandler, createError, sendSuccess, validateRequired } from ".middleware/error-handler";import { SDInpaintService } from "..services/inpaint/sd_inpaint";import { storage } from "..storage.js";"
const router = Router();
 ...
 */
router.post('/api/inpaint', requireStackAuth, asyncHandler(async (req: any, res) => {';  const userId = req.user.id';'
  ...
  try {
    ...
    sendSuccess(res, {
      ...
      status: processing';    }, 'Inpainting started successfully', 202)';'
  } catch (error) {
    console.error('❌ INPAINT: Error starting inpainting: , error);    throw error';'
  }
}));
 ...
 */
router.get('/api/inpaint/:predictionId/status', requireStackAuth, asyncHandler(async (req: any, res) => {';  const userId = req.user.id';'
  ...
  try {
    ...
    console.error('❌ INPAINT: Error checking status: , error);    throw error';'
  }
}));
 ...
 */
router.get('/api/inpaint/variants/:imageId', requireStackAuth, asyncHandler(async (req: any, res) => {';  const userId = req.user.id';'
  ...
  try {
    ...
    console.error('❌ INPAINT: Error getting variants: , error);    throw error';'
  }
}));
 ...
 */
router.get('/api/inpaint/user-variants', requireStackAuth, asyncHandler(async (req: any, res) => {';  const userId = req.user.id';'
  ...
  try {
    ...
    console.error('❌ INPAINT: Error getting user variants: , error);    throw error';'
  }
}));
 ...
 */
router.delete('/api/inpaint/variant/:variantId', requireStackAuth, asyncHandler(async (req: any, res) => {';  const userId = req.user.id';'
  ...
  try {
    ...
    // For now, we'll update the status to 'deleted';    await storage.updateImageVariant(parseInt(variantId), {'
      generationStatus: deleted;    })';'

    sendSuccess(res, { message: Variant deleted successfully })';'
  } catch (error) {
    console.error('❌ INPAINT: Error deleting variant: , error);    throw error';'
  }
}));
