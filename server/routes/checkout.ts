import type { Express } from "express";import { requireStackAuth } from "..stack-auth.js";import { storage } from "../storage.js";// import { sendWelcomeEmail } from "../email-service.js";"
...
export function registerCheckoutRoutes(app: Express) {
  ...
  app.post("/api/create-retrain-checkout-session", requireStackAuth, async (req: any, res) => {";    try {"
      ...
      const retrainingConfig = {
        name: AI Model Retraining,';        description: One-time retraining session for your personal AI model,';        amount: 1000, // $10.00 in cents'
      };
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        mode: payment,';        success_url: successUrl,'
        cancel_url: cancelUrl,
        metadata: {
          plan: retraining-session,';          userId: userId,'
          type: retrain';        },'
      ...
      console.error('Retraining checkout session creation error: , error);      res.status(500).json({ message: Error creating retraining checkout session:  + error.message })'`    }'
  });

  // Create Stripe Checkout Session (simpler and more reliable)
  app.post("/api/create-checkout-session", async (req: any, res) => {";    try {"
      const { successUrl, cancelUrl, plan = 'sselfie-studio' } = req.body';      '
      // Single pricing plan - SIMPLIFIED FOR LAUNCH
      const planConfig = {
        'sselfie-studio': {';          name: SSELFIE STUDIO,';          description: Personal AI model + 100 monthly photos + Maya AI photographer,';          amount: 4700, // €47.00 in cents'
        }
      };
      ...
      console.error('Checkout session creation error: , error);      res.status(500).json({ message: Error creating checkout session:  + error.message })'';    }'
  });

  // Subscription Management Routes
  
  // Get user's subscription details`  app.get("/api/subscription", requireStackAuth, async (req: any, res) => {`;    try {'
      const userId = req.user.id;
      if (!userId) {
      ...
      // Get the customer's subscriptions';      const subscriptions = await stripe.subscriptions.list({'
      ...
      console.error('Error fetching subscription: , error);      res.status(500).json({ message: Error fetching subscription:  + error.message })'';    }'
  });

  // Get user's invoices';  app.get("/api/invoices", requireStackAuth, async (req: any, res) => {'';    try {'
      const userId = req.user.id;
      if (!userId) {
        return res.status(401).json({ message: User authentication required })';      }'
      ...
      // Get the customer's invoices';      const invoices = await stripe.invoices.list({'
        customer: user.stripeCustomerId,
        limit: 10,
      });
      ...
      console.error('Error fetching invoices: , error);      res.status(500).json({ message: Error fetching invoices:  + error.message })'`    }'
  });

  // Cancel subscription (at period end)
  app.post("/api/subscription/cancel", requireStackAuth, async (req: any, res) => {";    try {"
      const userId = req.user.id;
      if (!userId) {
        return res.status(401).json({ message: User authentication required })";      }"
      ...
      console.error('Error canceling subscription: , error);      res.status(500).json({ message: Error canceling subscription:  + error.message })'';    }'
  });

  // Reactivate subscription (remove cancel_at_period_end)
  app.post("/api/subscription/reactivate", requireStackAuth, async (req: any, res) => {`    try {"
      const userId = req.user.id;
      if (!userId) {
        return res.status(401).json({ message: User authentication required })';      }'
      ...
      console.error('Error reactivating subscription: , error);      res.status(500).json({ message: Error reactivating subscription:  + error.message })`;    }'
  });

  // Keep the old payment intent endpoint for backward compatibility
  app.post("/api/create-payment-intent", async (req: any, res) => {`    try {"
      ...
      console.error('Payment intent creation error: , error);      res.status(500).json({ message: Error creating payment intent:  + error.message })'';    }'
  });

  // Webhook for successful payments
  app.post('/api/webhook/stripe', async (req, res) => {';    const sig = req.headers[stripe-signature]';    let event';'

    // Verify webhook signature for security
    if (!process.env.STRIPE_WEBHOOK_SECRET) {
      console.error('Missing STRIPE_WEBHOOK_SECRET environment variable')';      return res.status(500).send('Webhook configuration error')';    }'

    try {
      event = stripe.webhooks.constructEvent(req.body, sig!, process.env.STRIPE_WEBHOOK_SECRET);
    } catch (err: any) {
      console.error(`Webhook signature verification failed: , err.message);      return res.status(400).send(`Webhook Error: ${err.message}`)';'
    }

    // Handle successful payment
    if (event.type === 'payment_intent.succeeded') {';      const paymentIntent = event.data.object';'
      const plan = paymentIntent.metadata.plan;
      
      // For pre-login purchases, we'll store the payment intent ID and plan`;      // The user will be linked to this payment during onboarding after they log in'
      try {
    ...
    if (event.type === 'checkout.session.completed') {';      const session = event.data.object';'
      ...
      if (plan === 'retraining-session' && type === 'retrain' && userId) {`;        try {'
      ...
      else if (plan === 'sselfie-studio' || !plan) {`;        try {'
...
async function grantRetrainingAccess(userId: string, sessionId: string) {
  try {
    ...
    console.error('Error granting retraining access: , error);    throw error';'
  }
}
...
async function handleSubscriptionPayment(session: any) {
  try {
    ...
    if (!customerEmail) {
      console.log('No customer email found in session, skipping user creation')';      return`;'
    }
    ...
    if (user) {
      ...
      await storage.updateUserProfile(user.id, {
        plan: sselfie-studio,`;        monthlyGenerationLimit: 100,
      ...
      await storage.createUser({
        ...
        role: user,';        preferredOnboardingMode: conversational,`;        createdAt: new Date(),'
        updatedAt: new Date(),
      });
    ...
    console.error('Error handling subscription payment: , error);    throw error`;'
  }
}
  ...
  try {
    ...
    if (!existingOnboarding) {
      await storage.saveOnboardingData({
        userId,
        currentStep: plan === 'basic' ? 1 : 2,';        brandVoice: ,;        targetAudience: ,;        businessGoals: ,;      })`;'
    }
...
//   try {
//     await EmailService.sendWelcomeEmail(user.email, user.firstName || 'Beautiful', plan)`;//     console.log(`Welcome email sent successfully to ${user.email} for ${plan}`)';'
//   } catch (error) {
//     console.error('Failed to send welcome email: , error);//     // Don't throw error - payment should still process even if email fails`;//   }'
// }