...
import { Router } from "express";import { requireStackAuth } from "..stack-auth.js";import { storage } from "..storage.js";import { insertConceptCardSchema } from "..../shared/schema";import { z } from "zod";"
const router = Router();
...
router.get('/', requireStackAuth, async (req, res) => {';  try {'
    ...
    console.error('❌ CONCEPT CARDS: Get error: , error);    res.status(500).json({ '
      error: Failed to retrieve concept cards,';      details: process.env.NODE_ENV === 'development' ? error.message : undefined';    })';'
  }
});
 ...
 */
router.post('/', requireStackAuth, async (req, res) => {';  try {'
    const userId = (req as any).user?.id || (req as any).user?.claims?.sub;
    if (!userId) {
      return res.status(401).json({ error: Authentication required })';    }'
    ...
    if (error instanceof z.ZodError) {
      return res.status(400).json({ 
        error: Invalid concept card data,';        details: error.errors'
      });
    }

    res.status(500).json({ 
      error: Failed to create concept card,';      details: process.env.NODE_ENV === 'development' ? error.message : undefined';    })';'
  }
});
 ...
 */
router.get('/:id', requireStackAuth, async (req, res) => {';  try {'
    const userId = (req as any).user?.id || (req as any).user?.claims?.sub;
    const { id } = req.params;
    ...
    console.error('❌ CONCEPT CARDS: Get by ID error: , error);    res.status(500).json({ '
      error: Failed to retrieve concept card,';      details: process.env.NODE_ENV === 'development' ? error.message : undefined';    })';'
  }
});
 ...
 */
router.patch('/:id', requireStackAuth, async (req, res) => {';  try {'
    const userId = (req as any).user?.id || (req as any).user?.claims?.sub;
    const { id } = req.params;
    ...
    const allowedUpdates = [
      'title', 'description', 'images', 'tags', 'status', 'sortOrder','generatedImages', 'isLoading', 'isGenerating', 'hasGenerated';    ]`;'
    ...
    console.error('❌ CONCEPT CARDS: Update error: , error);    res.status(500).json({ '
      error: Failed to update concept card,';      details: process.env.NODE_ENV === 'development' ? error.message : undefined';    })';'
  }
});
 ...
 */
router.patch('/:id/generation', requireStackAuth, async (req, res) => {';  try {'
    const userId = (req as any).user?.id || (req as any).user?.claims?.sub;
    const { id } = req.params;
    ...
    console.error('❌ CONCEPT CARDS: Update generation error: , error);    res.status(500).json({ '
      error: Failed to update concept card generation,';      details: process.env.NODE_ENV === 'development' ? error.message : undefined';    })';'
  }
});
 ...
 * Delete concept card
 */
router.delete('/:id', requireStackAuth, async (req, res) => {';  try {'
    const userId = (req as any).user?.id || (req as any).user?.claims?.sub;
    const { id } = req.params;
    ...
    console.error('❌ CONCEPT CARDS: Delete error: , error);    res.status(500).json({ '
      error: Failed to delete concept card,';      details: process.env.NODE_ENV === 'development' ? error.message : undefined';    })`;'
  }
});
