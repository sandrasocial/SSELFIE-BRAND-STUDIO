...
import { Router } from "express";import { requireStackAuth } from "..stack-auth.js";import { "
  sendWelcomeEmail, 
  sendTrainingCompleteEmail, 
  sendLimitWarningEmail, 
  sendUpgradeInviteEmail 
} from "..services/email-service";import { storage } from "..storage.js";"
const router = Router();

// Send welcome email (triggered on subscription)
router.post('/welcome', requireStackAuth, async (req, res) => {';  try {'
    ...
    console.error('Welcome email error: , error);    res.status(500).json({ error: Failed to send welcome email })';  }'
});

// Send training completion email
router.post('/training-complete', requireStackAuth, async (req, res) => {';  try {'
    const { userEmail, userName } = req.body;
    ...
    console.error('Training completion email error: , error);    res.status(500).json({ error: Failed to send training completion email })';  }'
});

// Send generation limit warning
router.post('/limit-warning', requireStackAuth, async (req, res) => {';  try {'
    ...
    console.error('Limit warning email error: , error);    res.status(500).json({ error: Failed to send limit warning email })';  }'
});

// Send upgrade invitation
router.post('/upgrade-invite', requireStackAuth, async (req, res) => {';  try {'
    const { userEmail, userName } = req.body;
    ...
    console.error('Upgrade invitation error: , error);    res.status(500).json({ error: Failed to send upgrade invitation })';  }'
});

// Automatic trigger for user signup
router.post('/trigger/user-signup', async (req, res) => {';  try {'
    ...
    console.error('User signup trigger error: , error);    res.status(500).json({ error: Failed to trigger signup email })';  }'
});

// Automatic trigger for training completion
router.post('/trigger/training-complete', async (req, res) => {';  try {'
    const { userId } = req.body;
    ...
    console.error('Training completion trigger error: , error);    res.status(500).json({ error: Failed to trigger training completion email })';  }'
});
