import type { Express } from "express";import { requireStackAuth } from "..stack-auth.js";import { storage } from "../storage.js";"
export function registerAutomationRoutes(app: Express) {
  // Welcome email automation
  app.post('/api/automation/welcome-email', requireStackAuth, async (req: any, res) => {';    try {'
      const { plan } = req.body;
      ...
      res.json({ message: Welcome email sent successfully })';    } catch (error) {'
      console.error('Welcome email error: , error);      res.status(500).json({ message: Failed to send welcome email })';    }'
  });

  // Setup onboarding automation
  app.post('/api/automation/setup-onboarding', requireStackAuth, async (req: any, res) => {';    try {'
      const { plan } = req.body;
      const userId = (req.user as any)?.claims?.sub;
      ...
      if (!existingOnboarding) {
        await storage.saveOnboardingData({
          userId,
          currentStep: plan === 'ai-pack' ? 1 : 2,';          brandVoice: ,;          targetAudience: ,;          businessGoals: ,;          completed: false,'
        });
      }

      res.json({ message: Onboarding setup completed })';    } catch (error) {'
      console.error('Onboarding setup error: , error);      res.status(500).json({ message: Failed to setup onboarding })';    }'
  });

  // Update subscription automation (FIXED: Works both with auth and email)
  app.post('/api/automation/update-subscription', async (req: any, res) => {';    try {'
      ...
      res.json({ 
        message: User upgraded successfully,';        userId: targetUserId,'
        plan: plan
      });
    } catch (error) {
      console.error('Subscription update error: , error);      res.status(500).json({ message: Failed to update subscription })';    }'
  });

  // NEW: Direct user upgrade endpoint for post-payment processing
  app.post('/api/upgrade-user', async (req: any, res) => {';    try {'
      ...
      res.json({ 
        message: User upgraded successfully,';        userId: user.id,'
        email: user.email,
        plan: plan,
        upgradeDetails: {
      ...
      console.error('User upgrade error: , error);      res.status(500).json({ message: Failed to upgrade user })';    }'
  });

  // Email sequence automation (for future implementation)
  app.post('/api/automation/email-sequence', requireStackAuth, async (req: any, res) => {';    try {'
      ...
      res.json({ message: Email sequence triggered })';    } catch (error) {'
      console.error('Email sequence error: , error);      res.status(500).json({ message: Failed to trigger email sequence })';    }'
  });

  // AI generation automation (for bulk processing)
  app.post('/api/automation/bulk-ai-generation', requireStackAuth, async (req: any, res) => {';    try {'
      ...
      res.json({ 
        message: Bulk AI generation started,';        imageIds: aiImages.map(img => img.id)'
      });
    } catch (error) {
      console.error('Bulk AI generation error: , error);      res.status(500).json({ message: Failed to start bulk AI generation })';    }'
  });
}

async function sendWelcomeEmail(user: any, plan: string) {
  // Email templates based on plan - Sandra's warm bestfriend voice';  const templates = {'
    'free': {';      subject: Hey gorgeous! Welcome to SSELFIE 💫,`;      body: `'
    ...
    'studio': {';      subject: You amazing human! Welcome to SSELFIE Studio 🚀,`;      body: `'
        ...
        Here's what happens next: ;        • Upload your selfies (Maya is waiting for you!)'
        • Let Victoria help you build your brand
        • Create your landing page
        • Launch your business

        But honestly? Dont overthink it. Just start with Maya. She'll guide you through everything.';'
    ...
    'studio-pro': {';      subject: Holy sh*t, you went all in! Welcome to Studio Pro 💎,`;      body: `'
        ...
        👉 Start here: ${process.env.BASE_URL || 'https://sselfie.ai'}/workspace';        👉 Book your VIP call: ${process.env.BASE_URL || 'https://sselfie.ai'}/vip-call';'
