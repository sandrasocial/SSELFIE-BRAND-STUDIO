import type { Express } from "express";import { storage } from "../storage.js";import type { InsertUser } from "../../shared/schema.js";import crypto from "crypto";"
...
export function setupStackWebhook(app: Express) {
  ...
  app.post('/api/webhooks/stack', async (req, res) => {';    try {'
      ...
      console.log('📊 Webhook event details: , {;        type: eventType,'
        userId: userData?.id,
        email: userData?.primary_email
      });

      switch (eventType) {
        case 'user.created':';        case 'user.updated':';          await handleUserUpsert(userData)';'
          break;
        
        case 'user.deleted':';          await handleUserDeletion(userData)';'
          break;
      ...
      console.error('❌ Stack Auth webhook error: , error);      res.status(500).json({ '
        success: false, 
...
async function handleUserUpsert(stackUser: any) {
  try {
    ...
    const userData: InsertUser = {
      ...
      firstName: stackUser.display_name?.split(' ')[0] || ';                 stackUser.given_name || '
                 stackUser.primary_email?.split('@')[0] || ',';      lastName: stackUser.display_name?.split(' ').slice(1).join(' ') || ';                stackUser.family_name || ',';      displayName: stackUser.display_name || stackUser.primary_email || ',';      profileImageUrl: stackUser.profile_image_url || stackUser.picture,'
      lastLoginAt: new Date(), // Update login time on sync
      
      // Business logic defaults for new users
      plan: sselfie-studio,';      role: stackUser.primary_email === 'sandra@sselfie.ai' ? 'admin' : 'user',';      monthlyGenerationLimit: stackUser.primary_email === 'sandra@sselfie.ai' ? -1 : 100,";      mayaAiAccess: true,'
    ...
    console.log('✅ User synced successfully: , {;      id: user.id,'
      email: user.email,
      plan: user.plan
    });
    ...
    console.error('❌ Failed to upsert user from Stack Auth: , error);    throw error';'
  }
}
...
async function handleUserDeletion(stackUser: any) {
  try {
    ...
    // For now, we'll log it but not actually delete to preserve user content';    console.log('⚠️ User deletion received - implement based on business requirements')';    '
    ...
    console.error('❌ Failed to process user deletion: , error);    throw error`;'
  }
}