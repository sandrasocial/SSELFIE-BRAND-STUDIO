...
const getOidcConfig = (0, memoizee_1.default)(async () => {
    try {
        ...
        const config = await client.discovery(new URL(issuerUrl), process.env.REPL_ID);
...
async function setupAuth(app) {
    ...
    passport_2.default.serializeUser((user, cb) => {
        try {
            cb(null, user);
        }
        catch (error) {
    ...
    passport_2.default.deserializeUser((user, cb) => {
        try {
            cb(null, user);
        }
        catch (error) {
    ...
    app.get("/api/login", (req, res, next) => {
        ...
        function authenticateUser() {
            ...
            passport_2.default.authenticate(`replitauth:${hostname}`, {
                scope: ["openid", "email", "profile", "offline_access"],
    ...
    app.get("/api/callback", (req, res, next) => {
        ...
        if (req.query.error) {
            ...
            return res.redirect(`/?error=oauth_error&details=${encodeURIComponent(req.query.error)}`);
        }
        if (!req.query.code) {
    ...
    async function handleManualTokenExchange(req, res, next) {
        try {
