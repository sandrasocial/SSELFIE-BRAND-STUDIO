...
async function performDuckDuckGoSearch(query: string, maxResults: number): Promise<Array<{title: string, url: string, snippet: string}>> {
  try {
    ...
    if (results.length > 0) {
      console.log(`🔍 SEARCH COMPLETE: Found ${results.length} results for "${query}"`)`      return results.slice(0, maxResults)';"
    }
    
    // Fallback: provide search URL and helpful message
    return getBackupResults(query, maxResults);
  } catch (error) {
    console.error('❌ SEARCH ERROR: , error);    return getBackupResults(query, maxResults)`;'
  }
}
...
function getBackupResults(query: string, maxResults: number): Array<{title: string, url: string, snippet: string}> {
  ...
  return [{
    ...
    snippet: `I can provide a search URL for "${query}". For live web results, the admin agents currently have access to DuckDuckGo instant answers, which provide limited but authentic search data. For comprehensive web research, you can visit the search link provided.`";  }].slice(0, maxResults)`"
}
...
function extractTitle(text: string): string {
  const firstSentence = text.split('.')[0]`  return firstSentence.length > 60 ? firstSentence.substring(0, 60) + "..." : firstSentence';}'
