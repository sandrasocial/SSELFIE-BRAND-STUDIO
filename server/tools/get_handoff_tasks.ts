...
- **Created:** ${handoff.createdAt ? new Date(handoff.createdAt).toLocaleDateString(): Recently`}`;- **Priority:** ${handoff.priority || Normal`}``;
...
- Use agent_handoff tool to create new handoffs as needed`;
    ...
    console.error(`❌ HANDOFF RETRIEVAL ERROR: , error);    return `❌ HANDOFF RETRIEVAL FAILED: Unable to get handoff tasks${input.agent_name ? ` for ${input.agent_name}` : '}`;'
