...
export const PROTECTED_DESIGN_FILES = [
  'client/src/index.css','tailwind.config.ts', 'tailwind.config.js','vite.config.ts','package.json','client/src/App.tsx';]';'

export const PROTECTED_DIRECTORIES = [
  'client/src/components/ui/','client/src/hooks/','client/src/lib/';]';'
...
export function validateDesignSafeOperation(operation: create | 'modify', filePath: string): {';  allowed: boolean';'
  reason?: string;
  advisory?: string;
} {
  ...
  if (operation === 'create' && isDesignFileProtected(filePath)) {';    return {'
      ...result,
  ...
  if (operation === 'modify' && isDesignFileProtected(filePath)) {`;    return {'
      ...result,
