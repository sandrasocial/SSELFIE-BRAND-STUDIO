...
export class ModelRetrainService {
  ...
  static async restartTraining(userId: string): Promise<{ success: boolean; message: string; replicateModelId?: string }> {
    try {
      ...
      if (existingModel.trainingStatus !== 'completed') {`;        return {'
          success: false,
      ...
      const trainingData = {
        destination: `${process.env.REPLICATE_USERNAME || 'models`}/${newModelName}`,';        input: {'
      ...
      const response = await fetch('https://api.replicate.com/v1/trainings', {';        method: POST,';        headers: {'
      ...
      if (!response.ok) {
        ...
        console.error(`❌ Replicate training failed: , response.status, errorData);        return {
          success: false,
      ...
      await storage.updateUserModel(userId, {
        ...
        trainingStatus: training,`;        triggerWord: existingModel.triggerWord, // Keep existing trigger word
        trainedModelPath: `${process.env.REPLICATE_USERNAME || 'models`}/${newModelName}`,';        modelType: flux-standard,`;        updatedAt: new Date()'
      });
      ...
      console.error(`❌ Model retrain error: , error);      return {
        success: false,
