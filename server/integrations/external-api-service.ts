/**
 * EXTERNAL API SERVICE - SANDRA'S INTEGRATION HUB'; * Connects AI agents to Make, Flodesk, ManyChat, Instagram, and more'
 */
...
export class ExternalAPIService {
  ...
  static async getFlodeskSubscribers(): Promise<FlodeskSubscriber[]> {
    try {
      const response = await fetch('https://api.flodesk.com/v1/subscribers', {';        headers: {'
          'Authorization': `Basic ${Buffer.from(process.env.FLODESK_API_KEY + ":').toString('base64`)}`,'Content-Type': 'application/json`;        }'
      });
      ...
      console.error('Flodesk API Error: , error);      throw new Error('Failed to fetch Flodesk subscribers')';    }'
  }
  ...
  static async importSubscribersToSSELFIE(subscribers: FlodeskSubscriber[]): Promise<number> {
    ...
    for (const subscriber of subscribers) {
      try {
        ...
        const response = await fetch('http://localhost:5000/api/email-capture', {';          method: POST,';          headers: { 'Content-Type': 'application/json' },';          body: JSON.stringify({'
            email: subscriber.email,
            firstName: subscriber.first_name,
            lastName: subscriber.last_name,
            source: flodesk_import,';            plan: imported_subscriber`;          })'
        });
  ...
  static async createFlodeskEmailCampaign(subject: string, content: string, segmentTags?: string[]): Promise<string> {
    try {
      const response = await fetch('https://api.flodesk.com/v1/emails', {';        method: POST,';        headers: {'
          `Authorization": `Basic ${Buffer.from(process.env.FLODESK_API_KEY + ":").toString('base64`)}`,'Content-Type': 'application/json';        },"
        body: JSON.stringify({
          subject,
          content,
      ...
      console.error('Flodesk campaign creation error: , error);      throw new Error('Failed to create email campaign')`;    }'
  }
  ...
  static async getInstagramComments(mediaId?: string): Promise<InstagramComment[]> {
    try {
      ...
      console.error('Instagram API Error: , error);      throw new Error('Failed to fetch Instagram comments')`;    }'
  }
  ...
  static async replyToInstagramComment(commentId: string, message: string): Promise<boolean> {
    try {
      ...
      console.error('Instagram reply error: , error);      return false';'
    }
  }

  static async getInstagramAnalytics(timeframe: today | 'week' | 'month' = 'week'): Promise<any> {';    try {'
      const since = new Date();
      if (timeframe === 'week') since.setDate(since.getDate() - 7)';      if (timeframe === 'month') since.setMonth(since.getMonth() - 1)`;'
      ...
      console.error('Instagram analytics error: , error);      return {}';'
    }
  }
  ...
  static async getManychatSubscribers(): Promise<any[]> {
    try {
      const response = await fetch('https://api.manychat.com/fb/subscriber/findBySystemField', {';        method: POST,';        headers: {'
          'Authorization`: `Bearer ${process.env.MANYCHAT_API_TOKEN}`,'Content-Type': 'application/json';        },'
        body: JSON.stringify({
          field_name: email,';          field_value: * // Get all subscribers';        })'
      });

      const data = await response.json() as any;
      return data.data || [];
    } catch (error) {
      console.error('ManyChat API Error: , error);      throw new Error('Failed to fetch ManyChat subscribers')';    }'
  }

  static async sendManychtMessage(subscriberId: string, content: any): Promise<boolean> {
    try {
      const response = await fetch('https://api.manychat.com/fb/sending/sendContent', {';        method: POST,';        headers: {'
          'Authorization`: `Bearer ${process.env.MANYCHAT_API_TOKEN}`,'Content-Type': 'application/json';        },'
        body: JSON.stringify({
      ...
      console.error('ManyChat send error: , error);      return false';'
    }
  }
  ...
  static async getMakeScenarios(): Promise<MakeScenariosResponse> {
    try {
      const response = await fetch('https://www.make.com/api/v2/scenarios', {';        headers: {'
          'Authorization`: `Token ${process.env.MAKE_API_TOKEN}`,'Content-Type': 'application/json`;        }'
      });
      ...
      console.error('Make API Error: , error);      throw new Error('Failed to fetch Make scenarios')`;    }'
  }
  ...
  static async triggerMakeScenario(scenarioId: number, data?: any): Promise<boolean> {
    try {
      ...
      console.error('Make scenario trigger error: , error);      return false';'
    }
  }

  static async createMakeWebhook(name: string, triggerUrl: string): Promise<string | null> {
    try {
      const response = await fetch('https://www.make.com/api/v2/hooks', {';        method: POST,';        headers: {'
          'Authorization`: `Token ${process.env.MAKE_API_TOKEN}`,'Content-Type': 'application/json';        },'
        body: JSON.stringify({
          name,
          url: triggerUrl
      ...
      console.error('Make webhook creation error: , error);      return null`;'
    }
  }
