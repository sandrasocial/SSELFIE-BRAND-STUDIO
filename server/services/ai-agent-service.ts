...
interface ConversationState {
  step: 'idle' | 'awaiting_funnel_topic' | 'awaiting_copy_approval' | 'processing';
  intent: string;
  data: {
    funnelTopic?: string;
...
export class AIAgentService {
  private highLevelService: HighLevelService | null = null;
  private claude: Anthropic;
  private conversationState: ConversationState;

  constructor(
    private claudeApiKey: string,
    private highLevelApiKey?: string
  ) {
...
Return only valid JSON, no additional text.`;
   ...
   */
  private async handleContactCreation(parsedResponse: any): Promise<any> {
   ...
   */
  private async initiateFunnelCreation(parsedResponse: any, originalCommand: string): Promise<any> {
   ...
   */
  private async handleConversationStep(command: string): Promise<any> {
   ...
   */
  private async processFunnelTopic(topic: string): Promise<any> {
   ...
   */
  private async generateFunnelCopy(topic: string): Promise<any> {
    try {
...
Make the copy compelling, professional, and optimized for conversions. Include HTML formatting for structure.`;
   ...
   */
  private async processCopyApproval(response: string): Promise<any> {
   ...
   */
  private async createApprovedFunnel(): Promise<any> {
    if (!this.highLevelService) {
      this.conversationState.step = 'idle'; // Reset state
      return {
        success: false,
    ...
    try {
      ...
      const result = await this.highLevelService.createFunnel(
        funnelName!,
        landingPageCopy!,
        thankYouPageCopy!
      );
