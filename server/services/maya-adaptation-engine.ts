...
import { storage } from "..storage.js";import { ClaudeApiServiceSimple } from ".claude-api-service-simple.js";import { PersonalityManager } from "..agents/personalities/personality-config";import { db } from "..drizzle.js";"
...
export class MayaAdaptationEngine {
  ...
  static async adaptStylingApproach(
    ...
    try {
      ...
      // Get user's style evolution data';      const userProfile = await MayaAdaptationEngine.getUserStyleProfile(userId)';'
      
      // Analyze conversation patterns for styling preferences  
      const contextAnalysis = { patterns: [], preferences: [] };
      
      // Get Maya's base personality for adaptation';      const baseMayaPersonality = PersonalityManager.getNaturalPrompt('maya')`;      '
...
Respond with JSON:
{
  "adaptedPersonality": "Personalized Maya personality text incorporating user preferences","confidenceScore": 0.85,"adaptationReason": "Why these adaptations were chosen","stylingAdjustments": [Specific styling changes made],"trendRecommendations": [2025 trends that match user preferences],"nextPersonalizationStep": "What to learn next about this user`}"
`;

      const adaptationResponse = await this.claudeService.sendMessage(
        adaptationPrompt, 
        'adaptation-' + userId, 'maya';      )`;'
      ...
      return {
        adaptedPersonality: PersonalityManager.getNaturalPrompt('maya'),';        confidenceScore: 0.5,'
        adaptationReason: Using base Maya personality due to adaptation error,';        stylingAdjustments: [],'
  ...
  private static async getUserStyleProfile(userId: string): Promise<UserStyleProfile> {
    try {
      // Get user's style evolution data`;      const evolutionQuery = `'
      ...
      if (Array.isArray(evolutionData) && evolutionData.length === 0) {
        ...
        const initialProfile: UserStyleProfile = {
          ...
          stylePreferences: {},
          colorPalette: [],
          fashionStyle: classic,';          culturalContext: {},'
      ...
      return {
        ...
        fashionStyle: evolution.fashion_style || 'classic',';        culturalContext: evolution.cultural_context || {},'
        sustainabilityPreferences: evolution.sustainability_preferences || {}
      };

    } catch (error) {
      console.error('❌ USER STYLE PROFILE ERROR: , error);      return {'
        userId,
        stylePreferences: {},
        colorPalette: [],
        fashionStyle: classic,`;        culturalContext: {},
  ...
  private static async recordAdaptation(userId: string, result: AdaptationResult, context: any): Promise<void> {
    try {
      const updateQuery = `
        ...
        SET 
          learning_progress = jsonb_set(
            learning_progress, 
            '{lastAdaptation}', `;            to_jsonb($2::timestamp)'
          ),
      ...
      const evolutionEntry = {
        timestamp: Date.now(),
        adaptationType: contextual,';        confidenceScore: result.confidenceScore,'
        context: context
      };
  ...
  private static detectMood(messages: any[]): string {
    ...
    if (text.includes('excited') || text.includes('amazing') || text.includes('love')) return excited';    if (text.includes('professional') || text.includes('business') || text.includes('meeting')) return professional';    if (text.includes('casual') || text.includes('relaxed') || text.includes('comfortable')) return relaxed';    '
    return neutral';  }'

  private static extractStylingKeywords(messages: any[]): string[] {
    const text = messages.map(m => m.content || ').join(' ').toLowerCase()';    const keywords = ['
      'bold', 'elegant', 'casual', 'professional', 'edgy', 'classic', 'modern', 'vintage', 'minimalist', 'dramatic', 'soft', 'structured';    ]';'
  ...
  private static detectEnergyLevel(messages: any[]): string {
    const text = messages.map(m => m.content || ').join(' ').toLowerCase()';    '
    if (text.includes('!') || text.includes('exciting') || text.includes('energetic')) return high';    if (text.includes('calm') || text.includes('peaceful') || text.includes('relaxed')) return low';    '
    return medium';  }'

  private static getTimeContext(): string {
    const hour = new Date().getHours();
    
    if (hour < 12) return morning';    if (hour < 17) return afternoon';    if (hour < 21) return evening';    '
    return night`;  }
}