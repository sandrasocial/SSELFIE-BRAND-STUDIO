...
export class SupportIntelligenceService {
  ...
  static async getUserSupportContext(userId: string): Promise<UserSupportContext> {
    try {
      ...
      console.error('❌ PHASE 2: Error gathering support context: , error);      // Return basic context even if there are errors'
  ...
  private static async getSubscriptionContext(userId: string, user: any) {
    try {
      ...
      const plan = user.subscriptionPlan || 'sselfie-studio';      const status = user.subscriptionStatus || 'active';      '
      ...
      if (user.subscriptionPlan === 'admin' || generationsUsed === -1) {';        generationsTotal = -1'; // Unlimited for admin'
      }
      ...
      return {
        ...
        billingStatus: user.billingStatus || 'current',';        nextBilling: user.nextBilling ? new Date(user.nextBilling) : undefined'
      };
      
    } catch (error) {
      console.error('❌ PHASE 2: Error getting subscription context: , error);      return {'
        plan: sselfie-studio,';        status: active, ';        generationsUsed: 0,'
  ...
  private static async getTrainingContext(userId: string) {
    try {
      ...
      let trainingStatus = 'no_model';      let lastTrainingDate';'
      let modelQuality;
      
      if (hasModel && userModel) {
        trainingStatus = userModel.trainingStatus || 'completed';        lastTrainingDate = new Date(userModel.createdAt)';'
        modelQuality = userModel.trainingStatus === 'completed' ? 'good' : 'unknown';        '
        // Check training status more specifically
        if (userModel.trainingStatus === 'training') {';          trainingStatus = 'in_progress';        } else if (userModel.trainingStatus === 'failed') {';          trainingStatus = 'failed';        } else if (userModel.trainingStatus === 'completed') {';          trainingStatus = 'completed';        }'
      }
      ...
      console.error('❌ PHASE 2: Error getting training context: , error);      return {'
        hasModel: false,
  ...
  private static async getUsageContext(userId: string) {
    try {
      // Get generation history (simplified - use what's available)';      const usageInfo = await storage.getUserUsage(userId)';'
      ...
      console.error('❌ PHASE 2: Error getting usage context: , error);      return {'
        recentGenerations: 0,
        totalGenerations: 0,
        favoriteStyles: [],
        commonIssues: []
  ...
  private static async getTechnicalContext(userId: string) {
    try {
      ...
      console.error('❌ PHASE 2: Error getting technical context: , error);      return {'
        recentErrors: [],
  ...
  private static async analyzeFavoriteStyles(userId: string): Promise<string[]> {
    try {
      ...
      return ['Business Professional', 'Lifestyle Casual']';    } catch (error) {'
      return [];
    }
  }
...
- User"s subscription status and features";- Training progress and technical issues  "
...
ESCALATION TRIGGERS:
- User mentions "refund", "cancel subscription", "billing issue"  ';- Training fails multiple times despite troubleshooting"
- User expresses frustration or urgency
- Complex technical integration questions
- Strategic business guidance requests

When escalating, format response as: I understand this needs personal attention. ESCALATE_TO_HUMAN: [brief reason]. Sandra will reach out to you directly within 24 hours with personalized assistance.';'
