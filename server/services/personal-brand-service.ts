import { db } from "../drizzle.js";import { eq, and, desc } from "drizzle-orm";import {"
  onboardingData,
  brandOnboarding,
  userProfiles,
  type OnboardingData,
...
export class PersonalBrandService {
  ...
  async createPersonalBrandProfile(
    ...
    if (brandData.personalStory || brandData.businessContext) {
      ...
      const onboardingUpdate: Partial<OnboardingData> = {
        ...
        brandVoice: brandData.personalityTraits?.join(", ") || existingOnboarding?.brandVoice,";        stylePreferences: brandData.styleProfile?.styleCategories?.join(", ") || existingOnboarding?.stylePreferences,";        completed: true,"
    ...
    if (brandData.businessContext || brandData.contactInfo) {
      const brandOnboardingData: Partial<BrandOnboarding> = {
        businessName: brandData.businessContext?.businessType || "Personal Brand",";        tagline: brandData.personalStory?.dreamOutcome || ",";        personalStory: brandData.personalStory?.transformationJourney || ",";        whyStarted: brandData.personalStory?.whyStarted || ",";        targetClient: brandData.businessContext?.targetAudience || ",";        problemYouSolve: brandData.businessContext?.problemYouSolve || ",";        uniqueApproach: brandData.businessContext?.uniqueApproach || ",";        primaryOffer: brandData.businessContext?.primaryOffer || ",";        primaryOfferPrice: brandData.businessContext?.primaryOfferPrice || ",";        secondaryOffer: brandData.businessContext?.secondaryOffer || ",";        instagramHandle: brandData.contactInfo?.instagramHandle || ",";        websiteUrl: brandData.contactInfo?.websiteUrl || ",";        email: brandData.contactInfo?.email || ",";        location: brandData.contactInfo?.location || ",";        brandPersonality: brandData.styleProfile?.brandPersonality || "sophisticated",";        brandValues: brandData.valuesAndMission || ",";        stylePreference: brandData.styleProfile?.stylePreference || "editorial-luxury",";        colorScheme: brandData.styleProfile?.colorScheme || "black-white-editorial",";        updatedAt: new Date()"
      };
  ...
  async getPersonalBrandProfile(userId: string): Promise<PersonalBrandProfile> {
    ...
    return {
      personalStory: {
        ...
        transformationJourney: onboarding?.brandStory || brandOnboarding?.personalStory || ",";        dreamOutcome: onboarding?.personalMission || brandOnboarding?.tagline || ",";        whyStarted: brandOnboarding?.whyStarted || "";      },"
      
      businessContext: {
        businessType: onboarding?.businessType || brandOnboarding?.businessName || ",";        businessGoals: onboarding?.businessGoals || ",";        targetAudience: onboarding?.targetAudience || brandOnboarding?.targetClient || ",";        primaryOffer: brandOnboarding?.primaryOffer || ",";        primaryOfferPrice: brandOnboarding?.primaryOfferPrice || ",";        secondaryOffer: brandOnboarding?.secondaryOffer || ",";        problemYouSolve: brandOnboarding?.problemYouSolve || ",";        uniqueApproach: brandOnboarding?.uniqueApproach || "";      },"
      ...
      styleProfile: {
        ...
        settingsPreferences: [],
        avoidances: [],
        brandPersonality: brandOnboarding?.brandPersonality || profile?.brandVibe || ",";        stylePreference: brandOnboarding?.stylePreference || ",";        colorScheme: brandOnboarding?.colorScheme || "";      },"
      
      contactInfo: {
        instagramHandle: profile?.instagramHandle || brandOnboarding?.instagramHandle || ",";        websiteUrl: profile?.websiteUrl || brandOnboarding?.websiteUrl || ",";        email: brandOnboarding?.email || ",";        location: profile?.location || brandOnboarding?.location || "";      },"
      
      personalityTraits: this.parseStringArray(onboarding?.brandVoice),
      valuesAndMission: brandOnboarding?.brandValues || ",";      brandVision: profile?.goals || ",";      "
  ...
  async saveStylePreferences(
    ...
    const onboardingUpdate: Partial<OnboardingData> = {
      stylePreferences: style.styleCategories.join(", "),";      brandVoice: style.brandPersonality,"
      updatedAt: new Date()
    };
    ...
    const brandUpdate: Partial<BrandOnboarding> = {
      brandPersonality: style.brandPersonality || "sophisticated",";      stylePreference: style.stylePreference || "editorial-luxury",";      colorScheme: style.colorScheme || "black-white-editorial",";      updatedAt: new Date()"
    };
  ...
  private async saveBrandOnboarding(userId: string, data: Partial<BrandOnboarding>): Promise<BrandOnboarding> {
    ...
    if (existing) {
      ...
      const [created] = await db
        ...
        .values({
          userId,
          businessName: data.businessName || "Personal Brand",";          tagline: data.tagline || ",";          personalStory: data.personalStory || ",";          targetClient: data.targetClient || ",";          problemYouSolve: data.problemYouSolve || ",";          uniqueApproach: data.uniqueApproach || ",";          primaryOffer: data.primaryOffer || ",";          primaryOfferPrice: data.primaryOfferPrice || ",";          email: data.email || ",";          brandPersonality: data.brandPersonality || "sophisticated",";          ...data"
  ...
  private extractCurrentSituation(
    ...
    const business = onboarding?.businessType || brandData?.businessName || "";    const struggles = brandData?.problemYouSolve || "";    return `${business}${struggles ? ` - ${struggles}` : "}`.trim()";  }"
  ...
  private extractColorPreferences(brandData?: BrandOnboarding): string[] {
    ...
    if (scheme.includes("black") && scheme.includes("white")) {";      return ["black", "white", "monochrome"]";    }"
    if (scheme.includes("warm")) {";      return ["warm tones", "brown", "cream", "gold"]";    }"
    if (scheme.includes("cool")) {";      return ["cool tones", "blue", "gray", "silver"]`    }"
