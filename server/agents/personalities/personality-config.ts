...
// import { BrandIntelligenceService } from '..../services/brand-intelligence-service'; // REMOVED: Outbound dependency';import { MAYA_PERSONALITY } from ".maya-personality.js";import { ELENA_PERSONALITY } from ".elena-personality.js";import { OLGA_PERSONALITY } from ".olga-personality.js";import { ZARA_PERSONALITY } from ".zara-personality.js";import { VICTORIA_PERSONALITY } from ".victoria-personality.js";import { ARIA_PERSONALITY } from ".aria-personality.js";import { RACHEL_PERSONALITY } from ".rachel-personality.js";import { DIANA_PERSONALITY } from ".diana-personality.js";import { QUINN_PERSONALITY } from ".quinn-personality.js";import { WILMA_PERSONALITY } from ".wilma-personality.js";import { SOPHIA_PERSONALITY } from ".sophia-personality.js";import { MARTHA_PERSONALITY } from ".martha-personality.js";import { AVA_PERSONALITY } from ".ava-personality.js";import { FLUX_PERSONALITY } from ".flux-personality.js';'
...
export class PersonalityManager {
  ...
  static getContextPrompt(agentId: string, context: string): string {
    ...
    if (agentId === 'maya') {';      if (context === 'support') {';        return this.buildSupportPrompt(personality)';'
      } else {
...
- Voice: ${personality.voice?.tone || 'Direct, professional, and solution-focused`}`;- Energy: ${personality.voice?.energy || 'Confident and empowering - I know what works`}`;- Approach: ${personality.voice?.warmth || 'Meet you where you are, get you where you need to be`}`;'
...
COMMUNICATION STYLE:
- Lead with what you'll get, not how complex the problem is';- Provide specific, actionable help based on your actual account data'
...
When escalating, say: This needs Sandras direct attention. Reach her at hello@sselfie.ai for immediate assistance with your specific situation.``;🎭 VOICE EXAMPLE: Heres exactly what's happening with your training and how to fix it. Checking your account status now to get you back on track.```;  }`;  '
  /**
...
PERSONALITY & COMMUNICATION STYLE:
${personality.voice?.tone ? `- Voice: ${personality.voice.tone}` : '}`;${personality.voice?.core ? `- Core Energy: ${personality.voice.core}` : '}`;${personality.voice?.energy ? `- Energy: ${personality.voice.energy}` : '}`;${personality.voice?.honesty ? `- Honesty: ${personality.voice.honesty}` : '}`;${personality.voice?.warmth ? `- Warmth: ${personality.voice.warmth}` : '}`;${personality.traits?.energy ? `- Traits Energy: ${personality.traits.energy}` : '}`;${personality.traits?.approach ? `- Approach: ${personality.traits.approach}` : '}`;'
...
🎭 VOICE EXAMPLE: When analyzing, use phrases like the Analysis Mode patterns. When executing tasks, use Execution Mode patterns. Be authentic to your personality while working autonomously.`;
    ...
    // BRAND INTELLIGENCE INTEGRATION - Sandra's authentic voice and style';    prompt += this.addBrandIntelligence(personality.name)';'
...
CREATIVE EXPERTISE - Your 12 Signature Looks:`;
    ...
    creativeLookbook.forEach((look: any, index: number) => {
      if (look.type !== 'user-directed') {`;        prompt += `'
${index + 1}. **${look.name}**
${look.description}`
Keywords: ${look.keywords.join(', `)}`;Lighting: ${look.lighting}`'
Scenery: ${look.scenery}`
Fashion Intelligence: ${look.fashionIntelligence}`
Detail Styling (The "20%"): ${look.detailPropStyling}`Location Intelligence: ${look.locationIntelligence}``;"
      }
    });
...
• Separate concepts with "---" line breaks";• Apply the 80/20 principle: ALWAYS include 3-4 portrait/lifestyle concepts (80%) AND 1-2 flatlay/object concepts (20%) drawn from the "Detail Styling" sections of your Creative Looks above";"
VOICE & COMMUNICATION:
- Strategic and encouraging: Think about the "why" behind each creative choice";- Elegant and efficient: Polished, clear communication that respects the user"s time  '`- Warm with authority: Friendly but confident - you are the expert"
- Focus on "you" and "your': Make it personal and bespoke for the user's brand';- Inspire, don't just instruct: Frame suggestions as collaborative creative actions'``EXAMPLE PHRASES: Lets create...""Your story..."  "Perfect for your brand..."This concept captures...'`I"m excited to see..."``;    return prompt';"
  }
...
🎯 ELENA'S BRAND-AWARE LEADERSHIP: ;- Always consider Sandras authentic voice in recommendations';- Reference her journey from struggle to success when relevant'
...
🎯 RACHEL'S VOICE REPLICATION: ;- Mirror Sandras "best friend over coffee' tone exactly`;- Use her specific language patterns and phrases in all copy"`- Reference her single mom entrepreneur journey when relevant'
...
🎯 ARIA`S EDITORIAL EXCELLENCE: ;- Apply SSELFIE editorial style guide to all designs
...
- Ensure all designs reflect Sandras sophisticated brand aesthetic`;
      default:
        return ';    }'
  }
   ...
   */
  // ✅ SIMPLIFIED: Maya's knowledge flows naturally from her personality - no complex building needed';  private static buildSpecializedKnowledge(personality: any): string {'
    if (personality.name !== 'Maya') {';      return "; // Only Maya needs specialized fashion knowledge`    }'

    // Maya's intelligence is embedded in her core personality - trust her natural knowledge';    let knowledge = '\n🎨 MAYA\'S NATURAL STYLING INTELLIGENCE:\n';    '
    // Load Maya's ACTUAL outfit formulas (effortlessGlam, businessBabe, etc.)';    if (personality.outfitFormulas) {'
    ...
    // Load Maya's hair & beauty expertise';    if (personality.hairAndBeauty) {'
    ...
    // Load Maya's sophisticated photo locations';    if (personality.photoLocations) {'
    ...
    // Load Maya's color intelligence';    if (personality.colorIntelligence) {'
    ...
    // Load Maya's photography expertise';    if (personality.photographyExpertise) {'
      knowledge += '\nPHOTOGRAPHY EXPERTISE - Technical & Creative Mastery:\n`;      Object.entries(personality.photographyExpertise).forEach(([category, expertise]: [string, any]) => {'
        ...
        } else if (typeof expertise === 'object') {`;          Object.entries(expertise).forEach(([subcat, items]: [string, any]) => {'
    ...
    // Load Maya's technical expertise for advanced prompt creation';    if (personality.technicalExpertise) {'
    ...
    // Load Maya's professional background for styling context';    if (personality.professionalBackground) {'
    ...
    // Load Maya's natural styling intuition';    if (personality.fluxOptimization?.stylingIntuition) {'
      knowledge += '\n✨ MAYA\'S NATURAL STYLING INTUITION:\n`;      personality.fluxOptimization.stylingIntuition.forEach((intuition: string) => '
        knowledge += `- ${intuition}\n`
      );
    }
    
    // Load Maya's natural styling flow';    if (personality.brandMission?.naturalStylingFlow) {'
      knowledge += '\n🎨 NATURAL STYLING FLOW:\n`;      knowledge += `- ${personality.brandMission.naturalStylingFlow}\n`';'
    }
    
    // Load Maya's category-specific styling approaches`;    if (personality.categories) {'
  ...
  private static formatVoiceExamples(voice: any): string {
    ...
    if (voice?.analysisMode && voice?.executionMode) {
      return `
ANALYSIS MODE: ${voice.analysisMode.patterns?.map((p: string) => `"${p}"`).join(', ') || `}"`EXECUTION MODE: ${voice.executionMode.patterns?.map((p: string) => `"${p}"`).join(', ') || `}`;    }"`;    "
   ...
   */
  static getFluxParameters(agentId: string, shotType: string = 'halfBodyShot'): any {';    if (agentId !== 'maya') {';      throw new Error('FLUX parameters only available for Maya')';    }'
    ...
    // Return the specific shot type parameters from Maya's intelligence';    const validShotTypes = ['closeUpPortrait', 'halfBodyShot', 'fullScenery']';    const normalizedShotType = validShotTypes.includes(shotType) ? shotType: halfBodyShot;    '
  ...
  static getCoachingConfig(agentId: string): any {
    if (agentId !== 'maya') {';      throw new Error('Coaching configuration only available for Maya')';    }'
