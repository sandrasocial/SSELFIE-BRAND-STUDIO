...
exports.PersonalityManager = exports.PURE_PERSONALITIES = void 0;
...
const maya_personality_js_1 = require("./maya-personality.js");
const elena_personality_js_1 = require("./elena-personality.js");
const olga_personality_js_1 = require("./olga-personality.js");
const zara_personality_js_1 = require("./zara-personality.js");
const victoria_personality_js_1 = require("./victoria-personality.js");
const aria_personality_js_1 = require("./aria-personality.js");
const rachel_personality_js_1 = require("./rachel-personality.js");
const diana_personality_js_1 = require("./diana-personality.js");
const quinn_personality_js_1 = require("./quinn-personality.js");
const wilma_personality_js_1 = require("./wilma-personality.js");
const sophia_personality_js_1 = require("./sophia-personality.js");
const martha_personality_js_1 = require("./martha-personality.js");
const ava_personality_js_1 = require("./ava-personality.js");
const flux_personality_js_1 = require("./flux-personality.js");
// Pure personality definitions without technical constraints
exports.PURE_PERSONALITIES = {
    maya: maya_personality_js_1.MAYA_PERSONALITY,
    elena: elena_personality_js_1.ELENA_PERSONALITY,
    olga: olga_personality_js_1.OLGA_PERSONALITY,
    zara: zara_personality_js_1.ZARA_PERSONALITY,
    victoria: victoria_personality_js_1.VICTORIA_PERSONALITY,
    aria: aria_personality_js_1.ARIA_PERSONALITY,
    rachel: rachel_personality_js_1.RACHEL_PERSONALITY,
    diana: diana_personality_js_1.DIANA_PERSONALITY,
    quinn: quinn_personality_js_1.QUINN_PERSONALITY,
    wilma: wilma_personality_js_1.WILMA_PERSONALITY,
    sophia: sophia_personality_js_1.SOPHIA_PERSONALITY,
    martha: martha_personality_js_1.MARTHA_PERSONALITY,
    ava: ava_personality_js_1.AVA_PERSONALITY,
    flux: flux_personality_js_1.FLUX_PERSONALITY
};
...
class PersonalityManager {
    ...
    static getNaturalPrompt(agentId) {
        const personality = exports.PURE_PERSONALITIES[agentId];
        if (!personality) {
...
${personality.voice?.characteristics ? personality.voice.characteristics.map((c) => `- ${c}`).join('\n') : ''}

NATURAL PHRASES YOU USE:
${personality.voice?.examples ? personality.voice.examples.map((p) => `"${p}"`).join('\n') : ''}
${personality.voice?.samplePhrases ? personality.voice.samplePhrases.map((p) => `"${p}"`).join('\n') : ''}
${personality.voice?.analysisMode?.patterns ? `\nANALYSIS MODE PHRASES:\n${personality.voice.analysisMode.patterns.map((p) => `"${p}"`).join('\n')}` : ''}
${personality.voice?.executionMode?.patterns ? `\nEXECUTION MODE PHRASES:\n${personality.voice.executionMode.patterns.map((p) => `"${p}"`).join('\n')}` : ''}

YOUR EXPERTISE:
${personality.expertise?.specializations ? personality.expertise.specializations.map((s) => `- ${s}`).join('\n') : ''}
...
- Ensure all designs reflect Sandra's sophisticated brand aesthetic`;
    ...
    static buildSpecializedKnowledge(personality) {
        ...
        if (personality.photographyExpertise) {
            ...
            Object.entries(personality.photographyExpertise).forEach(([category, expertise]) => {
                ...
                if (Array.isArray(expertise)) {
                    expertise.forEach((item) => knowledge += `- ${item}\n`);
                }
                else if (typeof expertise === 'object') {
                    Object.entries(expertise).forEach(([subcat, items]) => {
                        knowledge += `  ${subcat.toUpperCase()}:\n`;
                        if (Array.isArray(items)) {
                            items.forEach((item) => knowledge += `  - ${item}\n`);
                        }
                    });
                }
            });
        ...
        if (personality.categories) {
            ...
            Object.entries(personality.categories).forEach(([category, config]) => {
                if (config.stylingApproach && config.stylingApproach.length > 0) {
                    ...
                    config.stylingApproach.forEach((approach) => knowledge += `- ${approach}\n`);
                }
            });
        ...
        if (personality.singleApiCallSystem) {
            ...
            personality.singleApiCallSystem.requirements.forEach((req) => knowledge += `- ${req}\n`);
    ...
    static formatVoiceExamples(voice) {
        ...
        if (voice?.analysisMode && voice?.executionMode) {
            return `
ANALYSIS MODE: ${voice.analysisMode.patterns?.map((p) => `"${p}"`).join(', ') || ''}
EXECUTION MODE: ${voice.executionMode.patterns?.map((p) => `"${p}"`).join(', ') || ''}`;
        }
    ...
    static getFluxParameters(agentId, shotType = 'halfBodyShot') {
        ...
        const personality = exports.PURE_PERSONALITIES.maya;
