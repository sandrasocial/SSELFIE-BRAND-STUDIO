...
export class AgentProposalGenerator {
  ...
  static async generateEmailCampaignProposal(context: any): Promise<any> {
    const proposal = {
      ...
      description: `Complete ${context.emailCount || 5}-email sequence to convert existing subscribers to €97/month SSELFIE Studio`,
      status: 'pending',
  ...
  static async generateAdCampaignProposal(context: any): Promise<any> {
    const proposal = {
      ...
      description: `Facebook/Instagram ads optimized for €97 conversions with €${context.dailyBudget || 50}/day budget`,
      status: 'pending',
      priority: context.priority || 'high',
      createdAt: new Date(),
      preview: {
    ...
    return `
      <div class="max-w-4xl mx-auto bg-white">
        <div class="text-center py-12 border-b">
          ...
          <div class="mt-8">
            <button class="bg-black text-white px-8 py-3 font-light">${context.ctaText || 'Start Your Studio €97/month'}</button>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="text-center py-20 px-6">
        ...
        <button className="bg-black text-white px-12 py-4 text-lg font-light hover:bg-gray-900 transition-colors">
          ${context.ctaText || 'Start Your Studio €97/month'}
        </button>
      </section>
    ...
    return `
      <div class="space-y-6">
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <p class="text-blue-800"><strong>CTA:</strong> Start Your Studio for €97/month</p>
        </div>
        <div class="grid md:grid-cols-2 gap-4">
          <div class="bg-gray-50 p-4 rounded">
    ...
    return `
      <div class="max-w-md mx-auto bg-white border rounded-lg shadow-sm">
        ...
        <div class="p-4">
          ...
          <button class="w-full bg-black text-white py-2 rounded text-sm">Start Your Studio €97/month</button>
        </div>
      </div>
    `;
  }
