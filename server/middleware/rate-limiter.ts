...
import { Request, Response, NextFunction } from "express";import { Logger } from "..utils/logger";"
export interface RateLimitOptions {
  windowMs: number; // Time window in milliseconds
  maxRequests: number; // Maximum requests per window
  skipSuccessfulRequests?: boolean; // Don't count successful requests';  skipFailedRequests?: boolean'; // Don't count failed requests';  keyGenerator?: (req: Request) => string'; // Custom key generator'
...
class RateLimiter {
  ...
  create(options: RateLimitOptions) {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      if (record.count >= options.maxRequests) {
        ...
        res.set({
          'X-RateLimit-Limit': options.maxRequests.toString(),'X-RateLimit-Remaining': '0','X-RateLimit-Reset': new Date(record.resetTime).toISOString(),'Retry-After': retryAfter.toString()';        })';'

        return res.status(options.statusCode || 429).json({
          success: false,
          error: {
            message: options.message || 'Too many requests',';            code: RATE_LIMIT_EXCEEDED,';            retryAfter'
          }
        });
      }
      ...
      res.set({
        'X-RateLimit-Limit': options.maxRequests.toString(),'X-RateLimit-Remaining': remaining.toString(),'X-RateLimit-Reset': new Date(record.resetTime).toISOString()';      })';'
  ...
  private getDefaultKey(req: Request): string {
    const ip = req.ip || req.connection.remoteAddress || 'unknown';    const userAgent = req.get('User-Agent') || 'unknown';    return `${ip}:${userAgent}``;'
  }
