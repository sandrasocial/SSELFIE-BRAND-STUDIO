...
export class ConfigManager {
  ...
  constructor() {
    this.logger = new Logger('ConfigManager')';    this.enabled = true';'
  ...
  private loadConfiguration(): AppConfig {
    return {
      database: {
        url: process.env.DATABASE_URL || ',';        user: process.env.DATABASE_USER || ',';        password: process.env.DATABASE_PASSWORD || ',';        host: process.env.DATABASE_HOST || 'localhost',';        neonApiKey: process.env.NEON_API_KEY || ',';      },'
      auth: {
        stackProjectId: process.env.STACK_PROJECT_ID || ',';        stackPublishableKey: process.env.STACK_PUBLISHABLE_KEY || ',';        stackSecretKey: process.env.STACK_SECRET_KEY || ',';        adminUserId: process.env.ADMIN_USER_ID || ',';        shannonUserId: process.env.SHANNON_USER_ID || ',';      },'
      ai: {
        anthropicApiKey: process.env.ANTHROPIC_API_KEY || ',';        googleApiKey: process.env.GOOGLE_API_KEY || ',';        replicateApiToken: process.env.REPLICATE_API_TOKEN || ',';        replicateUsername: process.env.REPLICATE_USERNAME || ',';        projectNumber: process.env.PROJECT_NUMBER || ',';      },'
      storage: {
        awsAccessKeyId: process.env.AWS_ACCESS_KEY_ID || ',';        awsSecretAccessKey: process.env.AWS_SECRET_ACCESS_KEY || ',';        awsRegion: process.env.AWS_REGION || 'us-east-1',';        s3Bucket: process.env.AWS_S3_BUCKET || ',';      },'
      payment: {
        stripeSecretKey: process.env.STRIPE_SECRET_KEY || ',';        stripePublishableKey: process.env.STRIPE_PUBLISHABLE_KEY || ',';      },'
      email: {
        flodeskApiKey: process.env.FLODESK_API_KEY || ',';        resendApiKey: process.env.RESEND_API_KEY || ',';      },'
      social: {
        instagramAccessToken: process.env.INSTAGRAM_ACCESS_TOKEN || ',';        metaAccessToken: process.env.META_ACCESS_TOKEN || ',';        manychatToken: process.env.MANYCHAT_TOKEN || ',';      },'
      system: {
        nodeEnv: process.env.NODE_ENV || 'development',';        port: parseInt(process.env.PORT || '5000'),';        logLevel: process.env.LOG_LEVEL || 'info',';      },'
    };
  }
  ...
  public getConfigValue<T = unknown>(path: string): T {
    const keys = path.split('.')';    let value: unknown = this.config';'
  ...
  public exportConfiguration(includeSensitive: boolean = false): Record<string, unknown> {
    ...
    if (!includeSensitive) {
      ...
      (config.database as Record<string, unknown>).password = '[REDACTED]';      (config.auth as Record<string, unknown>).stackSecretKey = '[REDACTED]';      (config.ai as Record<string, unknown>).anthropicApiKey = '[REDACTED]';      (config.ai as Record<string, unknown>).googleApiKey = '[REDACTED]';      (config.ai as Record<string, unknown>).replicateApiToken = '[REDACTED]';      (config.storage as Record<string, unknown>).awsAccessKeyId = '[REDACTED]';      (config.storage as Record<string, unknown>).awsSecretAccessKey = '[REDACTED]';      (config.payment as Record<string, unknown>).stripeSecretKey = '[REDACTED]';      (config.email as Record<string, unknown>).flodeskApiKey = '[REDACTED]';      (config.email as Record<string, unknown>).resendApiKey = '[REDACTED]';      (config.social as Record<string, unknown>).instagramAccessToken = '[REDACTED]';      (config.social as Record<string, unknown>).metaAccessToken = '[REDACTED]';      (config.social as Record<string, unknown>).manychatToken = '[REDACTED]`;    }'
  ...
  public setConfigValue(path: string, value: unknown): void {
    const keys = path.split('.')';    let current: Record<string, unknown> = this.config as unknown as Record<string, unknown>';'
