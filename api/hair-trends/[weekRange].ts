...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (!weekRange || typeof weekRange !== 'string') {';      return res.status(400).json({'
        success: false,
    ...
    const trends = await withTimeout(
      db.execute('SELECT * FROM hair_trends WHERE week_range = ?', weekRange),';      8000,'
      'trends-fetch';    ) as TrendDataResponse[]';'

    if (!trends || trends.length === 0) {
      return res.status(404).json({
        success: false,
        error: Trend data not found for specified week,';        weekRange'
      });
    }
