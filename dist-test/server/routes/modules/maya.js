...
router.post('/api/maya-chat', requireStackAuth, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const unifiedIntelligence = await unifiedMayaIntelligenceService.getUnifiedStyleIntelligence(userId, context, 'chat');
...
`;
        ...
        const mayaResponseObj = await claudeService.sendMessage(message, claudeHistory, mayaPersonality);
...
Analyze the image and respond with ONLY the motion prompt that perfectly captures and enhances what you see - no explanation, no additional text.`;
        ...
        const mayaVideoResponseObj = await claudeService.sendMessage(`${videoDirectorPrompt}\n\nImage URL for context: ${imageUrl}`, [], 'You are Maya, a professional AI creative director helping with video generation.');
...
router.post('/api/maya/heart-image', requireStackAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
    ...
    try {
        ...
        const galleryImage = await MayaChatPreviewService.heartImageToGallery(userId, imageUrl, prompt || 'Hearted from Maya chat', category || 'Maya AI');
        const responseData = {
            data: {
                galleryImage,
