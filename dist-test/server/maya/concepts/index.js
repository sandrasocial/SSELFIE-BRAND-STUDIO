...
async function handleGetConcepts(req, res, userId) {
    try {
        ...
        // Apply filters
        if (type) {
            query = query.where(and(eq(mayaConcepts.userId, userId), eq(mayaConcepts.type, type)));
        }
        if (status) {
            query = query.where(and(eq(mayaConcepts.userId, userId), eq(mayaConcepts.status, status)));
        }
        if (isTemplate !== undefined) {
            query = query.where(and(eq(mayaConcepts.userId, userId), eq(mayaConcepts.isTemplate, isTemplate)));
        }
        if (search) {
            query = query.where(and(eq(mayaConcepts.userId, userId), like(mayaConcepts.title, `%${search}%`)));
        }
        // Apply sorting
