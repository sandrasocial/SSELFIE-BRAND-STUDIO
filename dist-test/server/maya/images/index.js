...
async function handleGetImages(req, res, userId) {
    try {
        ...
        // Apply filters
        if (category) {
            query = query.where(and(eq(mayaImages.userId, userId), eq(mayaImages.category, category)));
        }
        if (isFavorite !== undefined) {
            query = query.where(and(eq(mayaImages.userId, userId), eq(mayaImages.isFavorite, isFavorite)));
        }
        if (isArchived !== undefined) {
            query = query.where(and(eq(mayaImages.userId, userId), eq(mayaImages.isArchived, isArchived)));
        }
        // Apply sorting
