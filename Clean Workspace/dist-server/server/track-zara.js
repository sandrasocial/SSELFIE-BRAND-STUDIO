...
async function trackZara() {
    ...
    try {
        ...
        const learningPatterns = await db
            .select()
            .from(agentLearning)
            .where(and(eq(agentLearning.agentName, 'zara'), eq(agentLearning.userId, '42585527')))
        ...
        const contextData = await db
            ...
            .where(and(eq(agentSessionContexts.agentId, 'zara'), eq(agentSessionContexts.userId, '42585527')))
        ...
        for (const ctx of contextData) {
            const contextObj = ctx.contextData;
