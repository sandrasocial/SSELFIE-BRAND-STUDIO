...
router.post('/api/send-welcome-email', requireStackAuth, asyncHandler(async (req, res) => {
...
router.post('/api/send-training-email', requireStackAuth, asyncHandler(async (req, res) => {
    const { email, firstName } = req.body;
    if (!email) {
        throw createError.validation('Email is required');
    }
