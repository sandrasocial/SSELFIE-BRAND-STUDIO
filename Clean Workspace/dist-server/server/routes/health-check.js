...
router.get('/health/ready', async (req, res) => {
    try {
        ...
        const allHealthy = checks.every(check => check.status === 'fulfilled' && check.value.status === 'healthy');
        if (allHealthy) {
            res.status(200).json({
                status: 'ready',
                timestamp: new Date().toISOString()
...
async function checkExternalServices() {
    try {
        ...
        const serviceResults = results.map((result, index) => result.status === 'fulfilled' ? result.value : {
