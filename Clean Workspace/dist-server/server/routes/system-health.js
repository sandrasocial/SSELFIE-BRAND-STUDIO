...
function calculateOverallHealth() {
    ...
    if (overallScore >= 95)
        ...
        status = 'critical';
    const alerts = [];
    if (scores.agents < 100)
        alerts.push('Some agents not responding');
    if (scores.insights < 90)
        alerts.push('Insight generation issues');
    if (scores.notifications < 95)
        alerts.push('Notification delivery problems');
