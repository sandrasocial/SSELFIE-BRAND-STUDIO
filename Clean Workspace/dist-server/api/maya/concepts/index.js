...
async function handleGetConcepts(req, res, userId) {
    try {
        ...
        const { type, search, page = 1, limit = 20, sortOrder = 'desc' } = queryParams;
        const offset = (page - 1) * limit;
        const whereConditions = [eq(mayaConcepts.userId, userId)];
        if (type) {
            whereConditions.push(eq(mayaConcepts.type, type));
        }
        if (search) {
            whereConditions.push(like(mayaConcepts.title, `%${search}%`));
        }
        const query = db.select()
            .from(mayaConcepts)
            .where(and(...whereConditions))
            .orderBy(sortOrder === 'desc' ? desc(mayaConcepts.createdAt) : asc(mayaConcepts.createdAt))
            .limit(limit)
            .offset(offset);
        const [concepts, totalCount] = await Promise.all([
            query,
            db.select({ count: mayaConcepts.id })
                .from(mayaConcepts)
                .where(and(...whereConditions))
        ]);
...
async function handleCreateConcept(req, res, userId) {
    try {
        ...
        const validConceptData = await validateMayaConcept(validatedData);
        const [newConcept] = await db.insert(mayaConcepts).values({
            userId: validConceptData.userId,
            title: validConceptData.title,
            description: validConceptData.description,
            prompt: validConceptData.prompt,
            type: validConceptData.type,
            metadata: validConceptData.metadata
        }).returning();
...
async function handleUpdateConcept(req, res, userId) {
    try {
        ...
        const [updatedConcept] = await db
            ...
            .where(and(eq(mayaConcepts.id, parseInt(conceptId)), eq(mayaConcepts.userId, userId)))
            .returning();
        if (!updatedConcept) {
