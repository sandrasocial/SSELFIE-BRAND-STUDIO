...
import { mayaModels } from '../../../shared/schema.js';
import { createInsertSchema } from 'drizzle-zod';
import { and, eq } from 'drizzle-orm';
const insertMayaModelsSchema = createInsertSchema(mayaModels);
...
async function handleGetModels(req, res, userId) {
    try {
        ...
        if (modelType) {
            query = query.where(and(eq(mayaModels.userId, userId), eq(mayaModels.modelType, modelType)));
        }
        if (status) {
            query = query.where(and(eq(mayaModels.userId, userId), eq(mayaModels.trainingStatus, status)));
        }
...
async function handleCreateModel(req, res, userId) {
    try {
        ...
        const [newModel] = await db.insert(mayaModels).values({
            userId: modelData.userId,
            modelType: modelData.modelType,
            trainingStatus: modelData.trainingStatus,
            trainingProgress: modelData.trainingProgress,
            metadata: {
                trainingImages: modelData.metadata.trainingImages || [],
                modelParameters: modelData.metadata.modelParameters || {},
                trainingLogs: modelData.metadata.trainingLogs || []
            }
        }).returning();
