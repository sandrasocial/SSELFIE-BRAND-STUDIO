...
export class TrainingCompletionHandler {
   ...
   */
  static async handleTrainingCompletion(
    userId: string,
    ...
    try {
      ...
      let extractedWeights: LoRAWeights | null = null;
      try {
   ...
   */
  private static async getOrCreateTriggerWord(userId: string): Promise<string> {
   ...
   */
  private static async sendModelReadyEmail(userId: string): Promise<void> {
    try {
   ...
   */
  private static async processBrandStrategy(userId: string): Promise<void> {
    try {
      const user = await storage.getUser(userId);
      ...
      const strategyData = JSON.parse(user.brandStrategyContext) as BrandStrategyContext;
